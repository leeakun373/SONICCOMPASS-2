...
- ✅ **数据顺序**: `normalized_coords[i]` 对应 `embeddings[i]`，索引一致性保持不变
- ✅ **全局UMAP**: 所有数据一起计算，可以发现跨类别相似性
...
1. **`cache/embeddings.npy`**: 向量矩阵 (N, 1024)
   - 每个文件的向量表示
   - 顺序与 metadata 一致

2. **`cache/metadata.pkl`**: 元数据列表，包含分类结果
   - 每个元数据包含 `category` 字段（CatID）
   - 顺序与 embeddings 一致

3. **`cache/coordinates_ucs.npy`**: UCS模式坐标 (N, 2)
   - 使用定锚群岛策略计算的坐标
   - `coordinates_ucs[i]` 对应 `embeddings[i]`

4. **`cache/coordinates_gravity.npy`**: Gravity模式坐标 (N, 2)
   - 使用全局UMAP计算的坐标
   - `coordinates_gravity[i]` 对应 `embeddings[i]`

5. **`data_config/ucs_coordinates.json`**: UCS坐标配置
   - 82个大类的固定坐标配置
   - 包含 `x`, `y`, `radius`, `gap_buffer` 等

### 7.2 数据一致性

**关键原则**:

- ✅ **索引一致性**: 所有数组（embeddings, metadata, coordinates）的索引 `i` 必须对应同一个文件
- ✅ **顺序一致性**: 数据顺序在整个流程中保持不变
- ✅ **分类结果一致性**: metadata 中的 `category` 字段是坐标计算的基础

**验证方法**:

```python
# 验证数据一致性
assert len(embeddings) == len(metadata) == len(coordinates_ucs)
for i in range(len(embeddings)):
    assert metadata[i]['filename'] == expected_files[i]
    assert embeddings[i].shape == (1024,)
    assert coordinates_ucs[i].shape == (2,)
```
...
**模块**: `ui/main_window.py`, `ui/visualizer/sonic_universe.py`

**功能**:

1. **加载数据**: 从缓存文件加载 embeddings, metadata, coordinates
2. **模式切换**: 支持 UCS 模式和 Gravity 模式切换
3. **坐标显示**: 根据当前模式加载对应的坐标文件
   - UCS模式: `coordinates_ucs.npy`
   - Gravity模式: `coordinates_gravity.npy`
4. **可视化渲染**: 使用 QPainter 和 Hexbin 渲染地图

**关键点**:

- ✅ **模式切换**: 切换模式时重新加载对应的坐标文件
- ✅ **视图状态**: 切换模式时重置缩放和中心点
- ✅ **坐标系统**: 显示固定的 0-3000 坐标系统（UCS模式）
- ✅ **鼠标交互**: 显示鼠标位置的坐标信息
...
```bash
# 1. 确保配置文件存在
# data_config/user_config.json 已配置数据库路径

# 2. 完整重建（包含向量化和坐标计算）
python rebuild_atlas.py

# 如果UCS坐标配置不存在，会报错：
# FileNotFoundError: UCS坐标配置文件不存在
# 请先运行: python tools/extract_category_centroids.py

# 3. 生成UCS坐标配置（从全局UMAP结果提取，如果有旧数据）
# 注意：首次运行如果没有旧坐标，需要手动定义或使用默认值
python tools/extract_category_centroids.py

# 4. 重新运行完整重建（现在有UCS配置了）
python rebuild_atlas.py
...
# 调整特定大类的 x, y, radius 值

# 3. 重新计算UCS模式坐标（向量化已完成，只需重新计算坐标）
...
```bash
# 1. 更新关键词映射
# 编辑 data_config/ucs_alias.csv，添加新关键词
...
# 4. 快速验证（使用测试脚本）
python tools/verify_subset.py WEAPON --limit 500 --mode ucs
...
数据顺序（索引 i）:
  ├─→ metadata[i]         # 阶段2: 导入
  ├─→ embeddings[i]       # 阶段3: 向量化
  ├─→ category[i]         # 阶段4: 分类
  ├─→ main_category[i]    # 阶段4: 主类别提取
  │
  ├─→ 锚点定义（阶段5）:
  │     ├─→ ucs_coordinates.json  # 固定坐标配置
  │     └─→ {"ANIMALS": {"x": 2747.0, "y": 189.0, "radius": 286.5}}
  │
  └─→ 坐标计算（阶段6）:
        ├─→ 按主类别分组 → category_groups[main_category[i]]
        ├─→ 局部UMAP计算 → local_coords
        ├─→ 归一化+平移 → final_coords_array[i]
        └─→ 最终坐标 = 锚点坐标 + (局部坐标 × 半径)
```
...
1. **数据准备阶段（阶段2-4）**:
   - 数据导入 → 向量化 → 分类
   - 索引 `i` 保持不变
   - 每个文件都有对应的 metadata, embeddings, category
...
embeddings (N, 1024)
    ↓ (分类)
metadata (添加 category 字段)
    ↓ (主类别提取)
main_category (82个大类)
    ↓ (锚点定义 - 可选，首次运行需要)
ucs_coordinates.json (固定坐标配置)
    ↓ (坐标计算 - UCS模式)
按主类别分组 → category_groups
    ↓ (局部UMAP)
local_coords (局部坐标)
    ↓ (归一化+平移)
final_coords_array (最终坐标)
    ↓ (保存)
coordinates_ucs.npy
    ↓ (UI加载)
可视化地图
...
- ✅ **彻底解决"大陆漂移"问题**: 使用定锚群岛策略，确保0%漂移
- ✅ **双模式架构**: UCS模式（硬规则+局部UMAP）和 Gravity模式（全局UMAP）完全解耦
- ✅ **数据顺序一致性**: 整个流程中索引 `i` 保持一致
- ✅ **可扩展性**: 支持200万+数据，并行计算优化

---

## 结论

本文档详细描述了 Sonic Compass 从数据库到UCS模式测试的完整数据流程。关键要点：

1. **数据顺序**: 索引 `i` 在整个流程中保持一致，`metadata[i]`, `embeddings[i]`, `coordinates[i]` 对应同一个文件

2. **锚点定义**: UCS坐标配置（`ucs_coordinates.json`）定义了82个大类的固定坐标，这些坐标是"锚点"，在局部UMAP计算之前已经定义

3. **坐标计算**: 基于分类结果分组数据，对每个大类独立运行局部UMAP，然后将局部坐标归一化并平移到锚点位置

4. **顺序关系**: 数据准备（导入→向量化→分类）→ 锚点定义（可选，首次需要）→ 坐标计算（必须基于锚点）

5. **双模式**: UCS模式使用固定坐标+局部UMAP，Gravity模式使用全局UMAP，两者完全独立

通过定锚群岛策略，Sonic Compass 确保了UCS模式的0%漂移，同时保留了Gravity模式的发现能力。
