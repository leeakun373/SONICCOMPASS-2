...
4. 验证阶段（可选）
   └─→ tools/verify_subset.py（微缩验证工具）
       ...
       ├─→ 处理流程：
       │   1. 从数据库查询包含关键词的数据
       │   2. 运行 DataProcessor 的分类逻辑
       │   3. 生成 UMAP 可视化散点图
       │   4. 打印分类报告
       │
       └─→ 输出：
           ├─→ PNG 图片（默认 verify_{keyword}.png）
           └─→ 控制台报告（分类来源统计）
```
...
### 4. `tools/verify_subset.py` - 微缩验证工具

**作用**：快速验证分类效果，无需运行全量构建

**输入**：
- 关键词（命令行参数，如 `AIR`, `WEAPON`）
- SQLite 数据库（默认 `test_assets/Sonic.sqlite`）
- `data_config/rules.json`（由 `generate_rules_json.py` 生成）
...
**使用时机**：
- 更新 `ucs_alias.csv` 和 `rules.json` 后，验证效果
- 调试分类逻辑时
- 快速测试新关键词映射时
